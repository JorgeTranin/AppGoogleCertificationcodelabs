package com.jorge.app_google_certification_codelabs.codelabs.core.notification
...
import com.jorge.app_google_certification_codelabs.databinding.FragmentNotificationBinding

private const val NOTIFICATION_ID = 0
private const val PRIMARY_CHANNEL_ID = "primery_notification_channel"
private const val ACTION_UPDADE = "ACTION_UPDATE_NOTIFICATION"
private const val ACTION_CANCEL = "ACTION_CANCEL_NOTIFICATION"
private const val ACTION_DELETE_ALL = "ACTION_DELETE_ALL_NOTIFICATION"
...
class NotificationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setupListeners()
        setupButtomStates(true, false, false)
    }

    private fun setupButtomStates(
        enableNotify: Boolean,
        enableUpdate: Boolean,
        enableCancel: Boolean
    ) {
        binding.notificationSimples.isEnabled = enableNotify
        binding.notificationAtualizar.isEnabled = enableUpdate
        binding.notificationCancelar.isEnabled = enableCancel
    }

    private fun setupListeners() {
        binding.notificationSimples.setOnClickListener { sendNotification() }
        binding.notificationCancelar.setOnClickListener { updateNotification() }
        binding.notificationAtualizar.setOnClickListener { cancelNotification() }
    }
    ...
    private fun cancelNotification() {
        ...
        setupButtomStates(enableNotify = true, enableUpdate = false, enableCancel = false)
    }

    private fun updateNotification() {
        // personalização dinamica da notificação adicionando um icone
        val androidImage = BitmapFactory.decodeResource(resources, R.drawable.baseline_circle_notifications_24)
        ...
        setupButtomStates(enableNotify = false, enableUpdate = false, enableCancel = true)
    }

    private fun sendNotification() {
        val builder = getNotificationBuilder()

        createNotificationAction(builder, NOTIFICATION_ID, ACTION_UPDADE, "Atualize")
        ...
        setupButtomStates(enableNotify = false, enableUpdate = true, enableCancel = true)
    }
    ...
    private fun registerNotificationReceiver() {
        ...
        notificationActionFilters.addAction(ACTION_UPDADE)
    ...
    inner class NotificationReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (intent.action) {
                ACTION_UPDADE -> updateNotification()
                ACTION_CANCEL -> {
                    notificationManager.cancel(NOTIFICATION_ID)
                    setupButtomStates(
                        enableNotify = true,
                ...
                ACTION_DELETE_ALL -> setupButtomStates(
                    enableNotify = true,
                    enableUpdate = false,
                    enableCancel = false
                )
